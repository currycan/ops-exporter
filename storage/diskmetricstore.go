...
const (
	...
	pushMetricHelp       = "Last Unix time when changing this group in the ops-exporter succeeded."
	pushFailedMetricName = "push_failure_time_seconds"
	pushFailedMetricHelp = "Last Unix time when changing this group in the ops-exporter failed."
...
func (dms *DiskMetricStore) Healthy() error {
	...
	// A ops-exporter that cannot be written to should not be
		...
		for name, tmf := range group.Metrics {
			...
			if exists {
				...
				if help, ok := dms.predefinedHelp[name]; ok && mf.GetHelp() != help {
					level.Info(dms.logger).Log("msg", "metric families overlap", "err", "Metric family has the same name as a metric family used by the ops-exporter itself but it has a different help string. Changing it to the standard help string. This is bad. Fix your pushed metrics!", "metric_family", mf, "standard_help", help)
